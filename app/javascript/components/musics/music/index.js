...
const RandomButton = styled(Columns.Column)`
	width: 130px;
`;
...
const Music = (props) => {
	...
	if (props.playing) {
		PlayButton = <FaStopCircle size='45px' className='has-text-white' onClick={() => props.setPlaying([])} />
	} else {
		PlayButton = <FaPlayCircle size='45px' className='has-text-white' onClick={() => props.setPlaying(props.song)} />
	}

	return (
		<Fragment>
			<Columns className='is-vcentered is-mobile is-centered is-12'>
